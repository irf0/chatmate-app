...
const ChatScreen = ({ navigation, route }) => {
  ...
  // console.log("who's typing", friendTyping);
    ...
    const getUserMessages = async () => {
      ...
      try {
        const unsubscribe = onSnapshot(conversationRef, (snapshot) => {
          ...
          const conversation = documentData?.conversation;
  ...
  const uploadSelfie = async (uri, name) => {
    ...
    try {
      ...
      if (!mediaUploading) {
        await sendImageMessage(friendUniqueID, url);
      }
    } catch (error) {
  ...
  const uploadAudioToFirebase = async (audioFileUri, duration) => {
    try {
      ...
      await sendAudioMessage(friendUniqueID, url, duration);
      setRecording(null);
    } catch (error) {
      console.error("Error while uploading file:", error);
      setAudioUploading(false);
  ...
  const sendImageMessage = async (senderId, photoURL) => {
    const loggedUserId = await AsyncStorage.getItem("docID");
    try {
      const messageDocRef = doc(FIREBASE_DB, "chats", combinedChatId);
      setMessageInput("");
      ...
      if (!mediaUploading && messageInput == "") {
        await updateDoc(
          messageDocRef,
          {
  ...
  const sendAudioMessage = async (senderId, audioURL, audioDuration) => {
    ...
    try {
      ...
      if (!audioUploading && messageInput == "") {
        await updateDoc(
          messageDocRef,
          {
            conversation: arrayUnion({
  ...
  const handleTyping = async (text) => {
    ...
    if (text || messageInput !== "") {
      updateDoc(messageDocRef, { user1isTyping: true, user2isTyping: true });
      setIsTyping(true);
    } else {
      updateDoc(messageDocRef, { user1isTyping: false, user2isTyping: false });
      setIsTyping(false);
    }
  };
                  ...
                  >
                    <View>
                      ...
                      {item.type === "image" && (
                        <TouchableWithoutFeedback
                          onLongPress={() =>
                            showAlert(item?.sendBy, item?.msgId)
                          }
                        >
                          <View>
                            <Image
